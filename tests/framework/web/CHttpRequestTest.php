...
class CHttpRequestTest extends CTestCase
{
    ...
    {
        $this->assertEquals($result, HttpRequest::parseAcceptHeader($header), $errorString);
    }

    /**
     * @covers       HttpRequest::compareAcceptTypes
    ...
    {
        $this->assertEquals($result, HttpRequest::compareAcceptTypes($a, $b), $errorString);
        // make sure that inverse comparison holds
        $this->assertEquals($result * -1, HttpRequest::compareAcceptTypes($b, $a), '(Inverse) ' . $errorString);
    }
